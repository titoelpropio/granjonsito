...
<table class="table table-striped table-bordered table-condensed table-hover"><tr>
	<td> <div class="pull-left"><h1>DEPOSITO DE CAJAS </h1></div> <input type="hidden" id="fecha">
	<div class="pull-right">@foreach($huevo_acumulado as $aux2)
			<font color="red" size="4">CANTIDAD DE HUEVOS: </font><input type="text" style="width:120px;height:40px;font-size:30px;text-align: center" value="{{$aux2->cantidad}}" id="huevo_acumulado" readonly="">
	@endforeach  <!--button class="btn-sm btn-info" onclick="actualizar_huevo()">MODIFICAR</button-->	</div></td>
				...
				@foreach($tipo_caja as $can)
				  <td style="background-color: {{ $can->color }}">
					...
					<input type="text" style="width:50px;height:30px;font-size:20px;text-align:center" readonly="" id="caja_acumulado{{$can->id}}" onkeypress="return bloqueo_de_punto(event)"> 	
					<input type="hidden" size="1" id="caja_acumulado_aux{{$can->id}}"><br><br>

					<font style=" background-color: white;">Cajas Por Dia:</font>
					<input type="number" style="width:55px;height:30px; font-size:20px;text-align:center" id="caja_diario{{$can->id}}" onkeypress='return bloqueo_de_punto(event)' onclick='extraer_id_tipo_caja({{$can->id}},1)'>
				...
				@foreach($tipo_caja as $can)
				  <td style="background-color: {{ $can->color }}">
					...
					<input type="number" style="width: 55px; height: 30px; font-size: 20px; text-align: center;" id="huevo_sobrante{{$can->id}}" onkeypress="return bloqueo_de_punto(event)" onclick='extraer_id_sobrante({{$can->id}},2)'><br>
			...
			<h1>HUEVOS DESCARTES</h1>
				...
				@foreach($tipo_huevo as $can)
				  <td>
				  	...
				  	<input type="number" style="width:80px;height:30px;font-size:20px;text-align:center" id="cantidad_tipo_huevo{{$can->id}}"  onkeypress="return bloqueo_de_punto(event)" onclick='extraer_id_tipo_huevo({{$can->id}},3)'>
				  	...
				  	<!--button class='btn btn-info' onclick="registrar_huevo({{$can->id}})" id="btnhuevo{{$can->id}}"><b>REGISTRAR</b></button-->
				  </td>
				@endforeach	
...
<script src="{{asset('js/cajadeposito_admin.js')}}"></script> 
@endsection
