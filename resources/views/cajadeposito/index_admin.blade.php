	...
	@endforeach  <button class="btn-sm btn-info" onclick="actualizar_huevo()">MODIFICAR</button>	</div></td>
				...
				@foreach($tipo_caja as $can)
				  <td style="background-color: {{ $can->color }}">
					...
					<!--button id="btnregistrar{{$can->id}}" class='btn btn-info' onclick="registrar_caja({{$can->id}})"><b>REGISTRAR</b></button><br-->

					<button class="btn-sm btn-warning" data-toggle='modal' data-target='#myModal' onclick="cargartabla_c({{$can->id}})"><b>DEVOLUCION</b></button>
